...
# Process data
df <- read_excel("Export.xlsx")
df_clean <- df %>%
  select(refYear, reporterISO, reporterDesc, fobvalue) %>%
  filter(!is.na(fobvalue))
full_data <- df_clean %>%
  group_by(refYear, reporterISO) %>%
  summarise(total_value = sum(fobvalue, na.rm = TRUE), .groups = "drop")
full_data <- full_data %>%
...
# Add predictors
pred <- read_excel("TotalExport.xlsx")
pred_clean <- pred %>%
  select(refYear, reporterISO, reporterDesc, fobvalue) %>%
  filter(!is.na(fobvalue))
pred_data <- pred_clean %>%
  group_by(refYear, reporterISO) %>%
  summarise(total_val = sum(fobvalue, na.rm = TRUE), .groups = "drop")
pred_data <- pred_data %>%
...
# Add log transformations of total export value and gdp
